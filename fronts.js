(async () => {
    const CONFIG = {
        API_URL: "http://178.156.199.40:3000",
        API_KEY: "key123", 
	...
	const renderSkeletonHome = () => {
		...
		for (let i = 0; i < 24; i++) {
			items += `
				<div class="col-xs-12 col-sm-6 col-md-4" style="margin-bottom: 2px;">
					<div class="sk-item">
		...
		document.body.innerHTML = getSkeletonStyle() + wrapInLayout(skeletonContent);
	};
	...
	const loadHome = async () => {
        ...
        const listHtml = shuffledData.map(item => `<dl class="listNews" style="margin-bottom:20px;margin-top:0px; "><small class="text-muted">${formatIndoDate(item.date_create)}</small><dt><a href="${getLink(item.slug)}" style="color:#086cab; text-decoration:none; font-weight:bold;">${toTitleCase(item.title)}</a></dt></dl>`).join('');
    ...
    const loadDetail = async (slug) => {
        ...
        const [resDetail, resBacklinks, resRelated] = await Promise.all([
            fetchAPI(`/api/manuals/detail/${slug}`),
            fetchAPI('/api/backlinks'),
            fetchAPI('/api/manuals') 
        ]);
		...
		if (r.data && r.data.length > 0) {
            r.data = r.data.sort(() => Math.random() - 0.5);
        }
        ...
        if (typeof content === 'string') {
            try {
                ...
                //console.error("Gagal parsing json_contents", e);
            }
        }
        ...
        if (Array.isArray(content)) {
                ...
                switch (obj.tag) {
                    case 'img':
                        return `<div style="margin: 20px 0; text-align:center;">
                                    <img src="${obj.link_image}" alt="${obj.alt}" style="max-width:100%; height:auto; border-radius:8px; ">
			...
			let cleanedContent = content.replace(/<a class="zoom start_zoom" href="#">[\s\S]*?<\/a>/g, '').replace(/<svg[\s\S]*?<\/svg>/g, '').replace(/<script[\s\S]*?>[\s\S]*?<\/script>/g, '');
			...
			bodyHtml = `
				<style>
					...
					.manual-scaler-fixed div.ocr {
						opacity: 0.7 !important;
						color: #333 !important;
					}
				</style>

				<div class="manual-outer-limit">
					<div class="manual-ratio-keeper">
						<div class="manual-scaler-fixed">
							${cleanedContent}
						</div>
					</div>
				</div>
                    ...
                    <article class="newsContent">
                        ...
                        <div style="margin-top: 40px; font-size: 16px; padding:20px; background:#f8f9fa; border-radius:8px;">
                            ...
                            <div style="line-height:2.2;">
                                ${b.data ? b.data.map(l => `<a href="${l.url}" target="_blank" style="margin-right:10px; color:#0088cc; text-decoration:none;">${toTitleCase(l.keyword)}</a>`).join('• ') : ""}
                            </div>
                        </div>
                    </article>
                </div>
